...
export async function callLLM<T>({
  ...
  maxTokens?: number;
  /** Override Groq model for this call. Use 'openai/gpt-oss-120b' for
  ...
  const tryGemini = async () => {
    ...
    throw new Error(`Gemini returned invalid JSON twice. Last output: ${raw2.slice(0, 200)}`);
  };
  ...
  const tryGroq = async () => {
    ...
    throw new Error(`Groq returned invalid JSON twice. Last output (${raw2.length} chars): ${raw2.slice(0, 600)}`);
  };
  ...
  if (primaryProvider === "groq") {
    try {
      ...
      try {
        ...
        throw new Error(`Both providers failed. Groq: ${errMsg(firstError)}. Gemini: ${errMsg(geminiError)}`);
      }
    }
  } else {
    try {
      ...
      firstError = e;
      if (!isRateLimited(e) && !isQuotaError(e) && !isTransient(e)) {
        throw e;
      }
      try {
        return await tryGroq();
      } catch (groqError) {
        throw new Error(`Both providers failed. Gemini: ${errMsg(firstError)}. Groq: ${errMsg(groqError)}`);
      }
    }
  }
}
