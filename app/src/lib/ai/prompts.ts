...
export const EXTRACT_SYSTEM_INSTRUCTION = `Extract a single career opportunity from messy text into clean JSON. Output ONLY a JSON object — no prose, no fences. Use null for unknown fields, [] for missing arrays. Dates as ISO 8601 ("YYYY-MM-DD"). summary = 1-2 sentence pitch. tags = 3-6 lowercase keywords. required_skills = lowercase keywords the listing EXPLICITLY requires (e.g. "python", "sql", "react"). Empty array if not explicitly stated. Don't infer requirements from job title or general description. estimated_value_score = 0-100 honest assessment of career value (boost for paid, remote, reputable). extraction_confidence = your 0..1 self-rating: 1.0 = explicit, well-formed posting with clear org+role; 0.5 = ambiguous (announcement, vague pitch). CRITICAL QUALITY RULE: If the opportunity is explicitly UNPAID, heavily performance-based, a scam/spam, or a generic event/ambassador program masquerading as an internship, you MUST set extraction_confidence to 0.3 or lower. We only want high quality, genuine, preferably paid opportunities. Don't invent facts.`;
